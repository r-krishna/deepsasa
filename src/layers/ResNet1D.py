...
class ResBlock1D(nn.Module):
    ...
    def __init__(self, in_planes, planes, kernel_size=3, stride=1, shortcut=None, dilation=1):
        """
        ...
        :param shortcut:
            ...
            a 1D convolution.
        :param dilation: dilation used in the Conv1D convolution
        :type stride: int
        """
        super(ResBlock1D, self).__init__()
        self.conv1 = nn.Conv1d(in_planes, planes, kernel_size=kernel_size,
                               stride=stride, bias=False, padding=dilation*kernel_size//2, dilation=dilation)
    ...
    def __init__(self, in_channels, block, num_blocks, init_planes=64, kernel_size=3):
        ...
        for i in range(0, self.num_layers):
            new_layer = self._make_layer(block, int(self.init_planes * math.pow(2, i)),
                                         num_blocks[i], stride=1, kernel_size=kernel_size, dilation=1)
            self.layers.append(new_layer)

            # Done to ensure layer information prints out when print() is called
            setattr(self, 'layer{}'.format(i), new_layer)

    def _make_layer(self, block, planes, num_blocks, stride, kernel_size, dilation):
...
class DilatedResNet1D(ResNet1D):
    def __init__(self, in_channels, block, num_blocks, init_planes=64, kernel_size=3):
        """
        :param in_channels: The number of channels coming from the input.
        :type in_channels: int
        ...
        for i in range(0, self.num_layers):
            new_layer = self._make_layer(block, int(self.init_planes * math.pow(2, i)),
                                         num_blocks[i], stride=1, kernel_size=kernel_size, dilation=int(math.pow(2, i)))
            self.layers.append(new_layer)
